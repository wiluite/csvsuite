...
mariadb_statement_backend::mariadb_statement_backend(
...
void mariadb_statement_backend::alloc()
{
    // nothing to do here.
}

void mariadb_statement_backend::clean_up()
{
...
void mariadb_statement_backend::prepare(std::string const & query,
...
mariadb_statement_backend::execute(int number)
{
            ...
            for (int i = 0; i != numberOfExecutions; ++i)
            {
                ...
                if (numberOfExecutions > 1)
                {
                            ...
                            static_cast<unsigned long>(query.size())))
                    {
                        ...
                        throw mariadb_soci_error(mysql_error(session_.conn_),
...
statement_backend::exec_fetch_result
mariadb_statement_backend::fetch(int number)
{
...
long long mariadb_statement_backend::get_affected_rows()
{
...
int mariadb_statement_backend::get_number_of_rows()
{
    return numberOfRows_ - currentRow_;
}

std::string mariadb_statement_backend::get_parameter_name(int index) const
{
    return names_.at(index);
}

std::string mariadb_statement_backend::rewrite_for_procedure_call(
...
int mariadb_statement_backend::prepare_for_describe()
{
    execute(1);
    justDescribed_ = true;
...
void mariadb_statement_backend::describe_column(int colNum,
...
data_type mariadb_statement_backend::to_data_type(db_type dbt) const
{
...
mariadb_standard_into_type_backend *
mariadb_statement_backend::make_into_type_backend()
{
    hasIntoElements_ = true;
    return new mariadb_standard_into_type_backend(*this);
}

mariadb_standard_use_type_backend *
mariadb_statement_backend::make_use_type_backend()
{
    hasUseElements_ = true;
    return new mariadb_standard_use_type_backend(*this);
}

mariadb_vector_into_type_backend *
mariadb_statement_backend::make_vector_into_type_backend()
{
    hasVectorIntoElements_ = true;
    return new mariadb_vector_into_type_backend(*this);
}

mariadb_vector_use_type_backend *
mariadb_statement_backend::make_vector_use_type_backend()
{
    hasVectorUseElements_ = true;
    return new mariadb_vector_use_type_backend(*this);
}
