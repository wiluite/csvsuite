...
mariadb_blob_backend::mariadb_blob_backend(mariadb_session_backend &)
    : details::trivial_blob_backend()
{
}

mariadb_blob_backend::~mariadb_blob_backend()
{
}
...
static unsigned char decode_hex_digit(char c)
{
    ...
    if (!( (i >= '0' && i <= '9') && (i >= 'a' && i <= 'f') ))
    {
        throw soci_error("MariaDB BLOB: Encountered invalid characters in hex representation");
    }
...
std::size_t mariadb_blob_backend::hex_str_size() const
{
...
void mariadb_blob_backend::write_hex_str(char *buf, std::size_t size) const
{
    if (size < hex_str_size())
    {
        throw soci_error("MariaDB BLOB: Provided buffer is too small to hold hex string");
    }
...
std::string mariadb_blob_backend::as_hex_str() const
{
...
void mariadb_blob_backend::load_from_hex_str(const char *str, std::size_t length)
{
