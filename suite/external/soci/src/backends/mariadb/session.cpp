...
private:
    mariadb_library()
    {
        ...
        {
            throw soci_error("Failed to initialize MariaDB library.");
        }
    }
...
mariadb_session_backend::mariadb_session_backend(
    connection_parameters const & parameters)
{
    mariadb_library::ensure_initialized();
...
#endif
    {
        ...
        clean_up();
        throw mariadb_soci_error(errMsg, errNum);
    }
}
...
bool mariadb_session_backend::is_connected()
{
    return mysql_ping(conn_) == 0;
}

void mariadb_session_backend::begin()
{
    hard_exec(conn_, "BEGIN");
}

void mariadb_session_backend::commit()
{
    hard_exec(conn_, "COMMIT");
}

void mariadb_session_backend::rollback()
{
    hard_exec(conn_, "ROLLBACK");
}

bool mariadb_session_backend::get_last_insert_id(
...
void mariadb_session_backend::clean_up()
{
    if (conn_ != NULL)
    {
        mysql_close(conn_);
...
mariadb_statement_backend * mariadb_session_backend::make_statement_backend()
{
    return new mariadb_statement_backend(*this);
}

mariadb_rowid_backend * mariadb_session_backend::make_rowid_backend()
{
    return new mariadb_rowid_backend(*this);
}

mariadb_blob_backend * mariadb_session_backend::make_blob_backend()
{
    return new mariadb_blob_backend(*this);
}
